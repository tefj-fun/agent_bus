...
class SkillAllowlistManager:
    ...
    async def check_permission(self, agent_id: str, skill_name: str) -> bool:
        """
    ...
    async def enforce_permission(self, agent_id: str, skill_name: str) -> None:
        """
        ...
        if not await self.check_permission(agent_id, skill_name):
            logger.warning(f"Permission denied: agent '{agent_id}' cannot use skill '{skill_name}'")
    ...
    async def remove_allowlist_entry(self, agent_id: str, skill_name: str) -> bool:
        """
        ...
        async with self.db_pool.acquire() as conn:
            ...
            if removed:
                logger.info(f"Allowlist entry removed: agent='{agent_id}', skill='{skill_name}'")

            return removed

    async def get_agent_allowed_skills(self, agent_id: str) -> List[str]:
        """
    ...
    async def remove_capability_mapping(self, capability_name: str, skill_name: str) -> bool:
        """
        ...
        async with self.db_pool.acquire() as conn:
            ...
            removed = result.split()[-1] != "0"
            if removed:
                logger.info(f"Capability mapping removed: '{capability_name}' -> '{skill_name}'")
