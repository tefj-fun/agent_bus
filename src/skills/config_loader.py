...
class AllowlistConfigLoader:
    ...
    async def load_from_yaml(
        ...
        stats['capability_mappings'] = await self._load_capability_mappings(
            capability_mappings
        )
    ...
    async def _load_allowlists(
        self,
        allowlists: Dict[str, List[Dict[str, Any]]]
    ) -> int:
        ...
        for agent_id, entries in allowlists.items():
            for entry in entries:
                ...
                if not skill_name:
                    logger.warning(
                        f"Skipping entry for {agent_id}: missing skill name"
                    )
                    continue
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to add allowlist entry for {agent_id}/{skill_name}: {e}"
                    )
        
        return count
    
    async def _load_capability_mappings(
        self,
        ...
        for capability_name, skills in mappings.items():
            for skill_entry in skills:
                ...
                if not skill_name:
                    logger.warning(
                        f"Skipping entry for {capability_name}: missing skill name"
                    )
                    continue
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to add capability mapping "
                        f"{capability_name}/{skill_name}: {e}"
                    )
