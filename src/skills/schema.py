...
class SkillMetadataSchema(BaseModel):
    ...
    capabilities: List[SkillCapability] = Field(
        default_factory=list, description="Capabilities provided by this skill"
    )

    # Tool requirements
    required_tools: List[SkillTool] = Field(
        default_factory=list, description="OpenClaw tools required by this skill"
    )
    ...
    entry_point: Optional[str] = Field(
        None, description="Main prompt file (default: skill.md, README.md, or prompt.md)"
    )

    # Compatibility
    min_python_version: Optional[str] = Field(
        None, description="Minimum Python version required (e.g., '3.10')"
    )
    ...
    license: Optional[str] = Field(
        None, description="License identifier (e.g., 'MIT', 'Apache-2.0')"
    )
...
class SkillsRegistrySchema(BaseModel):
    ...
    skills: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Map of skill name to metadata"
    )
...
EXAMPLE_SKILL_METADATA = {
    ...
    "author": "ComposioHQ",
    "capabilities": [
        {"name": "ui-design", "description": "Generate comprehensive UI/UX designs"},
        {"name": "design-systems", "description": "Create design systems and component libraries"},
    ],
