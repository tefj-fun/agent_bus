...
pytestmark = pytest.mark.skipif(
    True, reason="Requires database - run manually or in full integration tests"
)
...
class TestAgentSkillPermissions:
    ...
    async def test_execute_with_skill_permission_enforcement(self, agent_context, db_pool):
        ...
        with pytest.raises(SkillPermissionError):
            await agent.execute_with_skill("forbidden-skill", "Test prompt", {})
...
class TestMultipleAgents:
    ...
    @pytest.mark.asyncio
    async def test_different_agents_different_permissions(self, agent_context, db_pool):
