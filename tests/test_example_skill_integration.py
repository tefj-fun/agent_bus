...
pytestmark = pytest.mark.skipif(
    SKIP_DB_TESTS, reason="Requires database - run manually or in full integration tests"
)
...
class RestrictedAgent(BaseAgent):
    ...
    async def execute(self, task: AgentTask) -> AgentResult:
        return AgentResult(
            task_id=task.task_id, agent_id=self.agent_id, success=True, output={}, artifacts=[]
        )
...
def example_skills_dir():
    ...
    skill_json = {
        ...
        "capabilities": [
            {"name": "weather-query", "description": "Fetch current weather conditions"},
            {"name": "weather-forecast", "description": "Get multi-day forecasts"},
            {"name": "weather-analysis", "description": "Analyze weather patterns"},
        ],
        "required_tools": [
...
class TestCapabilityMapping:
    ...
    async def test_map_weather_capabilities(self, allowlist_manager):
        ...
        await allowlist_manager.add_capability_mapping(
            capability_name="weather-query", skill_name="weather-toolkit", priority=10
        )

        await allowlist_manager.add_capability_mapping(
            capability_name="weather-forecast", skill_name="weather-toolkit", priority=10
        )

        await allowlist_manager.add_capability_mapping(
            capability_name="weather-analysis", skill_name="weather-toolkit", priority=5
        )
    ...
    async def test_capability_priority_ordering(self, allowlist_manager):
        ...
        await allowlist_manager.add_capability_mapping(
            capability_name="data-fetch", skill_name="weather-toolkit", priority=5
        )

        await allowlist_manager.add_capability_mapping(
            capability_name="data-fetch", skill_name="other-toolkit", priority=10
        )
    ...
    async def test_multiple_skills_per_capability(self, allowlist_manager):
        ...
        await allowlist_manager.add_capability_mapping(
            capability_name="weather-query", skill_name="alternate-weather", priority=8
        )
...
class TestPermissionEnforcement:
    ...
    async def test_allow_agent_weather_skill(self, allowlist_manager):
        ...
        allowed = await allowlist_manager.check_permission("weather_agent", "weather-toolkit")
    ...
    async def test_deny_agent_weather_skill(self, allowlist_manager):
        ...
        allowed = await allowlist_manager.check_permission("restricted_agent", "weather-toolkit")
    ...
    async def test_wildcard_skill_access(self, allowlist_manager):
        ...
        allowed = await allowlist_manager.check_permission("admin_agent", "weather-toolkit")
        assert allowed is True
    ...
    async def test_specific_override_wildcard(self, allowlist_manager):
        ...
        # Allow all
        await allowlist_manager.add_allowlist_entry(
            agent_id="test_agent", skill_name="*", allowed=True
        )
        ...
        allowed = await allowlist_manager.check_permission("test_agent", "weather-toolkit")
        assert allowed is False
...
class TestSkillsManagerIntegration:
    ...
    async def test_load_skill_with_permission(self, skills_manager, allowlist_manager, db_pool):
        ...
        # Give permission
        await allowlist_manager.add_allowlist_entry(
            agent_id="weather_agent", skill_name="weather-toolkit", allowed=True
        )

        # Create manager with allowlist
        manager_with_perms = SkillsManager(skills_manager.skills_dir, db_pool=db_pool)

        # Should succeed
        skill = await manager_with_perms.load_skill("weather-toolkit", agent_id="weather_agent")
        assert skill is not None
    ...
    async def test_load_skill_without_permission(self, skills_manager, allowlist_manager, db_pool):
        ...
        # Deny permission
        await allowlist_manager.add_allowlist_entry(
            agent_id="restricted_agent", skill_name="weather-toolkit", allowed=False
        )
        ...
        with pytest.raises(SkillPermissionError):
            await manager_with_perms.load_skill("weather-toolkit", agent_id="restricted_agent")
    ...
    async def test_load_skill_no_allowlist(self, skills_manager):
        ...
        skill = await skills_manager.load_skill("weather-toolkit", agent_id="any_agent")
        assert skill is not None
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_weather_skill_workflow(
        self, skills_manager, allowlist_manager, db_pool
    ):
        """
        Complete workflow:
        ...
        for cap in skill.metadata.capabilities:
            await allowlist_manager.add_capability_mapping(
                capability_name=cap, skill_name="weather-toolkit", priority=10
            )
        ...
        loaded_skill = await manager_with_perms.load_skill(
            "weather-toolkit", agent_id="weather_agent"
        )
