...
class TestWorkflowStateMachine:
    ...
    def test_invalid_transitions_rejected(self):
        ...
        # Cannot go backwards
        assert not workflow.can_transition(
            WorkflowStage.DEVELOPMENT, WorkflowStage.ARCHITECTURE_DESIGN
        )

        # Cannot transition from terminal states
        assert not workflow.can_transition(WorkflowStage.COMPLETED, WorkflowStage.DELIVERY)
    ...
    def test_documentation_transition_to_pm_review(self):
        ...
        assert workflow.can_transition(WorkflowStage.DOCUMENTATION, WorkflowStage.PM_REVIEW)

    def test_support_docs_transition_to_pm_review(self):
        """Verify support docs stage can transition to PM review."""
        workflow = WorkflowStateMachine()

        assert workflow.can_transition(WorkflowStage.SUPPORT_DOCS, WorkflowStage.PM_REVIEW)

    def test_delivery_transition_to_completed(self):
        """Verify delivery stage can transition to completed."""
        workflow = WorkflowStateMachine()

        assert workflow.can_transition(WorkflowStage.DELIVERY, WorkflowStage.COMPLETED)
